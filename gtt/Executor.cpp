...
std::string traceMode(const Environment& env) {
    std::string mode = env.get("MYSH_TRACE");
...
bool traceEnabled(const Environment& env) {
...
void emitTraceSimple(const TraceEvent& evt) {
...
void emitTraceJson(const TraceEvent& evt) {
    std::cerr << "{\n";
...
void emitTraceGraph(const TraceEvent& evt) {
...
void emitTraceDot(const TraceEvent& evt) {
...
void emitTraceTimeline(const TraceEvent& evt) {
...
void emitTrace(const TraceEvent& evt, const std::string& mode) {
    if (mode == "json") {
        emitTraceJson(evt);
    } else if (mode == "graph") {
        emitTraceGraph(evt);
...
int executePipeline(const Pipeline& pipeline, Environment& env, bool background) {
    ...
    bool doTrace = traceEnabled(env);
    std::string mode = traceMode(env);
